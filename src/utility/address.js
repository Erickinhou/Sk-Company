...
export function getAddressFromOrder(orderData, getBilling = true) {
  ...
  const getFallback = (field) =>
    get(targetAddress, `${fallbackField}.${field}`, "");
  const getPreferred = (field) =>
    get(targetAddress, `${preferredField}.${field}`, getFallback(field));
...
export function getAddressForUpdate(orderData, updateAddressData, setBilling) {
  const { shipping_address, billing_address } = orderData;
  const targetAddress = cloneDeep(
    setBilling ? billing_address : shipping_address
  );

  const targetField = setBilling ? "address_billing" : "contact_address";

  const setAddress = (field, value) =>
    set(targetAddress, `${targetField}.${field}`, value);
